    ...
    <div class="flex flex-col items-center pb-10">
        <div>
            ...
            @if(count($user->clients)!==0)
                <div>
                    @if(is_null($user->clients[0]->birthday))
                        @php $user->clients[0]->birthday='Не указано' @endphp
                    @else
                        @php $user->clients[0]->birthday=date_format(date_create($user->clients[0]->birthday),'d.m.Y') @endphp
                    @endif
                    <span class="font-bold">Дата рождения:</span>
                    <span>{{$user->clients[0]->birthday}}</span>
                </div>
            @endif
