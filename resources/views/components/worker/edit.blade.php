@props(['user','posts','rooms','seances','workerSeances'])
                    ...
                    <x-form.select
                        ...
                        @foreach($posts as $post)
                            <option value="{{$post->id}}"
                                    @if($post->id === $user->workers[0]->post->id) selected @endif>{{$post->name}}</option>
                        @endforeach
                    </x-form.select>
                </div>
                <div class="mt-2">
                    ...
                    <x-form.select
                        ...
                        @foreach($rooms as $room)
                            <option value="{{$room->id}}"
                                    @if($room->id === $user->workers[0]->room->id) selected @endif>{{$room->name}}</option>
                        @endforeach
                    </x-form.select>
                </div>
                <div class="mt-2">
            ...
            <div>
                <div class="mt-2">
                    ...
                    @foreach($seances as $seance)
                        @php $checked=''; if(in_array($seance->id,$workerSeances)): $checked='checked'; endif;  @endphp
                        <x-form.checkbox name="seances[]"
                                         value="{{$seance->id}}" :checked="$checked">{{mb_substr($seance->name,0,5)}}</x-form.checkbox>
                    @endforeach
                </div>
            </div>
        </div>
